...
> 状态：**v0.5 开发目标定为网页版** · 共同设计讨论用文档，可直接在本文件修改
> 更新日期：2026-09-22
> 版本说明：v0.1 为"智能电商采购 Agent"（AI 自动下单），经可行性论证后转型为 **v0.2 智能导购 Agent**——AI 只做信息聚合与决策建议，交易 100% 由用户完成，合规风险大幅降低；**v0.3 价格情报优化**——联盟 API 字段级能力扩充（PLUS 价/百亿补贴/促销利益点）、新增"优惠类型获取策略"、明确禁用评论区爬虫；**v0.4 网页版定位**——开发目标调整为"网页版（桌面优先）"，小程序/App 降级为远期路线；对话方式支持图片输入（模型多模态）；Demo 定为全品类；前端技术栈暂未确定、后端技术栈不锁死；**v0.5 推荐商品功能**——新增"无目标商品"场景：用户只提需求（如"3000 元以内降噪耳机"）时，跨平台搜索候选商品并按画像/预算给出 Top N 推荐卡片，与既有"指定商品款式对比"流程并列，共用双价测算与领券指引
...
一句话定位：**网页版购物决策助手**——两类核心场景：①用户对某商品的多款式不知如何抉择时，Agent 获取各款式信息与用户画像，做介绍与推荐；②用户没有明确目标商品、只提需求（如"3000 元以内降噪耳机"）时，Agent 跨平台搜索候选商品并按画像与预算给出**推荐商品 Top N 卡片**；同时结合各平台活动价格，告诉用户如何参加活动拿到最低价。**下单这一步交给用户自己完成。**
...
- [x] 桌面端为第一优先级
- [x] **推荐商品（无目标商品场景）纳入核心流程**：按需求跨平台搜索候选商品 + 画像/预算匹配 + Top N 推荐卡片（v0.5 新增）
...
|-------|------|------|------|------|------------|
| A1 理解/路由 | 主管 | 意图解析（款式对比 / **推荐商品** 分流）、画像提取、任务路由、对话管理 | 用户消息+历史 | 结构化任务清单+画像 | 否 |
| A2 商品情报 | 工人·感知 | 商品定位 + SKU 款式采集 + **候选商品搜索（推荐场景用）** | 商品标识/关键词/需求描述 | 款式信息矩阵 / **候选商品列表** | ✅ 联盟 API + 详情接口 |
| A3 价格情报 | 工人·感知 | 实时查价、双价测算、会员价/活动价 | 款式列表 | 价格矩阵（公开价/领券价/会员价/活动价/券信息） | ✅ 联盟查券 API |
| A4 款式对比 | 工人·推理 | 款式差异对比、规格补全、**候选商品粗筛/打分（推荐场景用）** | 款式矩阵+RAG 规格库 | 结构化对比结论 / **推荐候选排序** | 否 |
| A5 推荐引导 | 工人·推理 | 画像匹配、推荐生成（含 **Top N 推荐卡片**）、领券指引 | 对比结论+价格矩阵+画像 | 最终回复（含免责） | 否 |
...
> **推荐商品场景的接口用法（v0.5）**：推荐流程由 `search_product(需求关键词)` 起步拿到候选列表（如"降噪耳机"跨平台 Top 候选），再对候选逐个调用 `get_price_with_coupon` / `get_coupon_info` 做双价测算，与画像/预算打分排序后输出 Top N。`search_product` 同时服务"指定商品"与"推荐商品"两类场景。
...
- 卡片式结果（推荐商品 Top N 卡片、推荐结论、款式对比表、双价、领券步骤按钮）
...
**场景定义**：用户没有明确目标商品，只提出需求描述（如"3000 元以内降噪耳机""学生党性价比键盘"）。区别于既有"指定商品→款式对比"路径，本功能从**品类/需求关键词**出发，产出**推荐商品 Top N 卡片**。

**流程定位**：推荐商品是独立入口流程，与款式对比流程共用数据源适配层、双价测算与领券指引能力；用户在推荐卡片中选择某款后，可无缝进入"款式对比"流程继续深挖。

**推荐排序依据（画像 + 价格 + 需求，A4 粗筛打分）**：

| 排序因子 | 权重 | 说明 |
|---------|------|------|
| 预算符合度 | 高 | 最低到手价 ≤ 用户预算优先；超预算需明确提示 |
| 画像匹配 | 高 | 价格敏感→优先券后低价；品质优先→优先旗舰/高配；学生→学生友好渠道 |
| 平台偏好 | 中 | 用户常用平台（淘宝/京东/拼多多）优先展示 |
| 价格竞争力 | 中 | 同款跨平台最低到手价（双价取 B 价） |
| 口碑/销量参考 | 低 | 联盟 API 销量字段（不引入评论区爬虫，保持 v0.3 边界） |

**Top N 输出规范**：
- 数量：默认 Top 3（可配置），候选池先按需求关键词 `search_product` 收敛
- 每张卡片包含：商品名 / 一句话推荐理由（对应排序因子）/ 三平台双价 / 领券指引入口
- 若候选不足或预算内无结果：明确告知并给出预算调整建议，不硬凑推荐
- 推荐理由必须可追溯到画像或价格数据（延续"信息透明、不误导"原则）

**前端承载**：网页版对话中输出"推荐商品卡片流"（同 3.3 卡片式结果）；首页可增设"热门推荐"入口作为推荐场景的快捷进入。

---

## 4. 核心流程

**流程 A：指定商品 → 款式对比（既有主流程）**
...
**流程 B：推荐商品（无目标商品，v0.5 新增）**

```mermaid
flowchart LR
    P["① 用户提需求 如3000元内降噪耳机"] --> Q["② 意图分流+画像提取 A1"]
    Q --> R["③ 候选商品搜索 search_product 联盟API"]
    R --> S["④ 候选粗筛与打分 画像+预算+价格 A4"]
    S --> T["⑤ 双价测算 公开价/领券价 A3"]
    T --> U["⑥ 推荐Top N卡片 含依据"]
    U --> V["⑦ 用户选一款 进入流程A③ 或直接领券下单"]
    V --> W["⑧ 领券指引/下单 平台App"]
    W --> X["⑨ 反馈与知识回流 RAG更新"]
```

> 全程系统不产生任何交易动作；第 ⑧ 步由用户在平台 App 内自助完成。流程 B 与流程 A 共享 A3 双价测算、A5 领券指引与 RAG 知识层。
...
3. **定义 Demo 核心流程**：双流程闭环——流程 A「咨询 → 款式对比 → 双价展示 → 领券指引」；流程 B「提需求 → 候选搜索 → Top 3 推荐卡片 → 领券指引」
4. **输出物**：项目目录结构 + 核心接口定义（含 `search_product` 推荐场景返回规范）+ 数据库表结构（含推荐记录表）+ 前端页面清单（全品类 Mock：首页 / 对话 / 款式对比 / 领券指引 / 推荐商品卡片）
